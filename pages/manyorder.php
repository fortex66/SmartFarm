...
// 가격에 따라 데이터베이스에서 정보 가져오기 (가격 기준으로 오름차순 정렬)
$sql = "SELECT c.*
FROM crop c
LEFT JOIN (
    SELECT Crop_cropID, SUM(amount) AS total_sales
    FROM orderdetail
    GROUP BY Crop_cropID
) s ON c.cropID = s.Crop_cropID
ORDER BY total_sales DESC, c.cropID;
";
